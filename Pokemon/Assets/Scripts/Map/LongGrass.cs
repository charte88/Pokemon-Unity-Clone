...
public class LongGrass : MonoBehaviour
{
    ...
    void OnTriggerEnter2D(Collider2D col)
    {
        if (col.GetComponent<Movement>())
        {
            ...
            {
                if (gm != null)
                    StartCoroutine(gm.EnterBattle(Rarity.VeryRare));
            }
            else if (p < r * 100)
            {
                if (gm != null)
                    StartCoroutine(gm.EnterBattle(Rarity.Rare));
            }
            else if (p < sr * 100)
            {
                if (gm != null)
                    StartCoroutine(gm.EnterBattle(Rarity.SemiRare));
            }
            else if (p < c * 100)
            {
                if (gm != null)
                    StartCoroutine(gm.EnterBattle(Rarity.Common));

            }
            else if (p < vc * 100)
            {
                if (gm != null)
                    StartCoroutine(gm.EnterBattle(Rarity.VeryCommon));
            }
        }
    }
}
